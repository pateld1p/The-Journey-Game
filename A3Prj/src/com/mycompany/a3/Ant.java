...
public class Ant extends Moveable implements IFoodie{
	...
	private Ant(Point location, int heading, int speed) {
		super(40, location, 255, 0, 0, heading, speed);	
	...
	public static Ant getAnt(Point location, int heading, int speed) {
		if (ant == null) 
			ant = new Ant(location, heading, speed); 	// create the Ant if not created already 
		return ant;
	}
	...
	public void draw(Graphics g, Point pCmpRelPrnt) {
		...
		g.fillArc((int) (pCmpRelPrnt.getX() + this.getLocation().getX()), (int) (pCmpRelPrnt.getY() + this.getLocation().getY()), this.getSize(), this.getSize(), 0, 360);		
	}
